...
(async function () {
    ...
    const createMemberCard = (m, form, formIndex, revealLevel) => {
        ...
        if (revealLevel >= 3) {
            ...
            if (form && formIndex !== undefined) url += `${url.includes('?') ? '&' : '?'}form=${formIndex}`;
            ...
            const frameHtml = fPath ? `<div class="cheki-frame" style="background-image:url('${window.fixPath(fPath)}');"></div>` : '';
