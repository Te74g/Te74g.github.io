...
/**
 * Builds and injects the header and footer into the document placeholders.
 * Returns a promise that resolves when the injection is complete.
 */
export async function renderLayout() {
    const config = getSiteConfig();
    // Do not render layout on maintenance page
    if (window.location.pathname.includes('maintenance.html')) return;

    // Calculate relative path depth automatically from current URL
  ...
  <footer class="site-footer">
    ...
    if (dirSegment) {
        document.querySelectorAll('.pc-nav .nav-item, .menu-link').forEach(link => {
            ...
            // Simple match: if the dir segment is roughly in the href (e.g., 'cast')
